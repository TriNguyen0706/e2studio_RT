...
# Pre-build step: run RASC to generate project content if configuration.xml is changed
	...
	set_property(DIRECTORY APPEND PROPERTY
		...
		"${CMAKE_CURRENT_BINARY_DIR}/rasc_timestamp.txt"
	)
    if (EXISTS "${CMAKE_CURRENT_BINARY_DIR}/rasc_timestamp.txt" )
		file(TIMESTAMP "${CMAKE_CURRENT_BINARY_DIR}/rasc_timestamp.txt" TS_LAST_CG "%s" UTC)
		file(TIMESTAMP "${CMAKE_CURRENT_SOURCE_DIR}/configuration.xml" TS_CONFIG "%s" UTC)
		message(STATUS "Configuration.xml timestamp  : ${TS_CONFIG}")
    	message(STATUS "Content generation timestamp : ${TS_LAST_CG}")
	else()
		SET(TS_LAST_CG "0")
		SET(TS_CONFIG "1")
	endif()

    if (TS_CONFIG LESS TS_LAST_CG)
        message(STATUS "Skipping content generation since configuration.xml is not updated since the last content generation")
	else()
		message("Running RASC for generating project ${PROJECT_NAME} content since modification is detected in configuration.xml:")

        if (EXISTS "${CMAKE_CURRENT_BINARY_DIR}/rasc_timestamp.txt")
           file(REMOVE "${CMAKE_CURRENT_BINARY_DIR}/rasc_timestamp.txt")
        endif()
		...
		if (resultVar AND NOT resultVar EQUAL 0)
			...
			file(TOUCH "${CMAKE_CURRENT_BINARY_DIR}/rasc_timestamp.txt")
		endif()
	endif()
endif()
...
target_compile_options(${PROJECT_NAME}.elf
                       PRIVATE
                       $<$<CONFIG:Debug>:
                           $<$<COMPILE_LANGUAGE:C>:${RASC_C_DEBUG_FLAGS}>
                           $<$<COMPILE_LANGUAGE:CXX>:${RASC_CXX_DEBUG_FLAGS}>
                           $<$<COMPILE_LANGUAGE:ASM>:${RASC_ASM_DEBUG_FLAGS}>
                       >
...
if (NOT IDE_SUPPORTS_PRE_POST_BUILD_ACTIONS)
	...
	add_custom_command(
	    ...
	    COMMAND
	        echo ${RASC_EXE_PATH} -nosplash --launcher.suppressErrors ${RASC_GEN_BUNDLE_CMD} --devicefamily ra --compiler GCC --toolchainversion ${CMAKE_C_COMPILER_VERSION} --buildconfiguration ${CMAKE_BUILD_TYPE} ${CMAKE_CURRENT_SOURCE_DIR}/configuration.xml ${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}.elf ${CMAKE_CURRENT_SOURCE_DIR}/UserNscApiFiles.txt
	    COMMAND
	        ${RASC_EXE_PATH} -nosplash --launcher.suppressErrors ${RASC_GEN_BUNDLE_CMD} --devicefamily ra --compiler GCC --toolchainversion ${CMAKE_C_COMPILER_VERSION} --buildconfiguration ${CMAKE_BUILD_TYPE} ${CMAKE_CURRENT_SOURCE_DIR}/configuration.xml ${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}.elf ${CMAKE_CURRENT_SOURCE_DIR}/UserNscApiFiles.txt 2> rasc_cmd_log.txt
	)
